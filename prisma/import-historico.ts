...
async function main() {
  // 1. Analistas a partir de los usuarios del Excel de Calidad
  ...
  // 2. Tareas
  const tareasRaw: Record<string, string | null>[] = JSON.parse(fs.readFileSync('/tmp/tareas.json', 'utf-8'))
  let tareasCreadas = 0
  ...
  // 3. Ingresos
  const ingresosRaw: Record<string, string | number | null>[] = JSON.parse(fs.readFileSync('/tmp/ingresos.json', 'utf-8'))
  ...
  // 4. Despachos
  const despachosRaw: Record<string, string | number | null>[] = JSON.parse(fs.readFileSync('/tmp/despachos.json', 'utf-8'))
