...
export async function POST(req: NextRequest) {
  ...
  // Con laboratorio interno, el protocolo es el propio número de muestra
  // (después de derivar el estado, ver derivarEstado).
  ...
  try {
    ...
    // Dos cargas simultáneas pueden llevarse el mismo próximo número.
    if (e instanceof Prisma.PrismaClientKnownRequestError && e.code === 'P2002') {
      return NextResponse.json({ error: 'Ese número de muestra ya está usado. Recargá el formulario para obtener el siguiente.' }, { status: 409 })
    }
    throw e
  }
}
